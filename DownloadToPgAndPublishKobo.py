...
# KOBO-GEOSERVER VARIABLES FOR POSTGIS
pg_workspace_name_kobo = config['PostGIS Workspace Store']['workspace_name_kobo']
pg_store_name_kobo = config['PostGIS Workspace Store']['store_name_kobo']
no_of_times_published_kobo = int(config['PostGIS Workspace Store']['publish_count_kobo'])
            ...
            for submission in data['results']:
                ...
                for key in list(submission):
                    ...
                    if key in self.fields and key == 'ODKUUID':
                        submission['odkuuid'] = submission['ODKUUID']
                        del submission['ODKUUID']
...
# OPEN POSTGIS LAYER AS VECTOR LAYER
